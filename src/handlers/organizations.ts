...
import { validateBody, get, getBySlug, create, deleteBySlug, updateBySlug } from "@/models/organizations";
...
export default new Hono<{ Bindings: Bindings }>()
  ...
  .post("/", validateBody, async (c) => {
    ...
    try {
      ...
      if (e.message === "D1_ERROR: UNIQUE constraint failed: organizations.slug: SQLITE_CONSTRAINT") {
        return c.redirect(`/v1/organizations/${body.slug}`, 303);
      }
    }
  })
  .put("/:slug", validateBody, async (c) => {
    ...
    if (!entries.length) {
      return c.redirect(`/v1/organizations/${slug}`, 303);
    }
